...
class iouEval:
    ...
    def addBatch(self, x, y):   #x=preds, y=targets
        ...
        if (self.ignoreIndex != -1): 
            ignores = y_onehot[:,self.ignoreIndex].unsqueeze(1) # ignores 是一个大小为 (batch_size, 1, height, width) 的张量，表示忽略的类别
            x_onehot = x_onehot[:, :self.ignoreIndex]           # (batch_size, num_classes, height, width) 的 one-hot 张量
