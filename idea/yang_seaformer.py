...
class Injecttion(nn.Module):
    ...
    def forward(self,semantics_input,local_input):
        ...
        mul_input = torch.sigmoid(mul_input)
        mul_input = F.interpolate(mul_input,size=(64,128),mode="bilinear",align_corners=False)
        add_input = self.add_semanyics(semantics_input)
        add_input = F.interpolate(add_input,size=(64,128),mode="bilinear",align_corners=False)
...
class Encoder(nn.Module):
    def __init__(self, num_classes,first_channel = 16):
        ...
        self.initial_block = DownsamplerBlock(3,first_channel)

        self.layers = nn.ModuleList()

        for x in range(0, 2):
            self.layers.append(SS_nbt_module(first_channel,1))
        

        self.layers.append(DownsamplerBlock(first_channel,32))
        ...
        self.layers.append(DownsamplerBlock(24,2*first_channel))

        for x in range(0, 1):    
            self.layers.append(SS_nbt_module(2*first_channel,1))
            self.layers.append(SS_nbt_module(2*first_channel,2))
            self.layers.append(SS_nbt_module(2*first_channel,5))
            self.layers.append(SS_nbt_module(2*first_channel,9))
        
        for x in range(0, 1):    
            self.layers.append(SS_nbt_module(2*first_channel,2))
            self.layers.append(SS_nbt_module(2*first_channel,5))
            self.layers.append(SS_nbt_module(2*first_channel,9))
            self.layers.append(SS_nbt_module(2*first_channel,17))
        
        self.layers_16 = nn.Sequential(
            DownsamplerBlock(2*first_channel,4*first_channel),
            SS_nbt_module(4*first_channel,1),
            SS_nbt_module(4*first_channel,1)
            
        )
        self.layers_32 = nn.Sequential(
            DownsamplerBlock(4*first_channel,8*first_channel),
            SS_nbt_module(8*first_channel,1)
        )
        
        self.transformer_1 = transformer_Block(dim = 8*first_channel, key_dim=first_channel, num_heads=4, mlp_ratio=4, attn_ratio=2, drop_path=0.1)
        self.transformer_2 = transformer_Block(dim = 8*first_channel, key_dim=first_channel, num_heads=4, mlp_ratio=4, attn_ratio=2, drop_path=0.1)
        
        
        
        self.fusion = Conv2d_BN(in_channel=6*first_channel,out_channel=4*first_channel,kernelsize=3,stride=1,padding=1)
        
        self.injection = Injecttion(semantics_channel=8*first_channel,local_channal=4*first_channel)
        ...
        self.output_conv = nn.Conv2d(4*first_channel, num_classes, 1, stride=1, padding=0, bias=True)
    ...
    def forward(self, input, predict=False):
        ...
        for layer in self.layers:
            output= layer(output)    # (batch_size,32,H/8,W/8)
            
        output_16 = self.layers_16(output)  # (batch_size,64,H/16,W/16)
        ...
        output_16 = F.interpolate(output_16,size=(H//8,W//8),mode="bilinear",align_corners=False)
        output = torch.cat([output,output_16],dim=1)
        output= self.fusion(output)
        output = self.injection(output_32,output)
